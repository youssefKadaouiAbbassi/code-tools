...
if ! command -v jq >/dev/null 2>&1; then
  printf '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"pre-destructive-blocker: jq not found in PATH — install jq to enable command screening"}}\n'
  exit 0
fi
...
# Patterns require the dangerous arg to be a STANDALONE token, not a prefix —
# otherwise we'd block legitimate `rm -rf /tmp/foo`, `rm -rf .git`, etc.
check_pattern 'rm[[:space:]]+-[a-zA-Z]*r[a-zA-Z]*f[[:space:]]+/([[:space:]]|;|$)' "Blocked: recursive force delete targeting root path"
check_pattern 'rm[[:space:]]+-[a-zA-Z]*r[a-zA-Z]*f[[:space:]]+~([[:space:]]|;|$)' "Blocked: recursive force delete targeting home directory"
check_pattern 'rm[[:space:]]+-[a-zA-Z]*r[a-zA-Z]*f[[:space:]]+\.([[:space:]]|;|$)' "Blocked: recursive force delete targeting current directory"
check_pattern 'rm[[:space:]]+-[a-zA-Z]*r[a-zA-Z]*f[[:space:]]+\*([[:space:]]|;|$)' "Blocked: recursive force delete with wildcard"
